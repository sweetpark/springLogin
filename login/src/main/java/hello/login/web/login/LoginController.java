...
public class LoginController {
    ...
    public String login(@Valid @ModelAttribute("loginForm") LoginForm form, BindingResult bindingResult, HttpServletResponse response){
        ...
        //DB까지 확인후 해야하기 떄문에 -> 필드오류 (fieldError) 로 사용못하고, ObjectError를 이용해야한다 (reject())
        if(loginMember == null){
            bindingResult.reject("loginFail", "아이디 또는 비밀번호가 맞지 않습니다.");
            return "login/loginForm";
        }

        //로그인 성공 처리 TODO -> 쿠키를 만들어서 client 로 전달

        // 쿠키에 시간 정보를 주지 않으면 세션 쿠키 ( 브라우저 종료시 모두 종료 )
        Cookie idCooikie = new Cookie("memberId", String.valueOf(loginMember.getId()));
        response.addCookie(idCooikie);
